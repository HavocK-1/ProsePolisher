...
// These functions inject the Gremlin instructions directly into the system prompt
// role of the chat template, which ensures proper formatting for each model and
// resolves the "model-specific prompt template" issue where some models don't
// attend to post-history injections or treat them inconsistently.

async function injectGremlinSystemPrompt(promptId, content) {
    const context = getContext();
    if (!context || typeof context.setExtensionPrompt !== 'function') {
        console.warn(`[ProjectGremlin] setExtensionPrompt not available in this SillyTavern build.`);
        return false;
    }
    try {
        // Clear any previous prompt with the same id first
        try { await context.setExtensionPrompt(promptId, '', extension_prompt_types.NONE, 0, false, extension_prompt_roles.USER); } catch (_) { /* ignore */ }
        // Inject as user role: position=IN_CHAT (relative to chat history),
        // depth=0 (at the very end of chat, i.e. the last message position),
        // scan=false, role=USER.
        // Using the numeric extension_prompt_roles.USER constant (not the
        // string 'user') because setExtensionPrompt coerces the role via
        // Number(role ?? SYSTEM). The string 'user' was becoming NaN -> SYSTEM,
        // which with prompt post-processing = None gets silently dropped.
        await context.setExtensionPrompt(promptId, content, extension_prompt_types.IN_CHAT, 0, false, extension_prompt_roles.USER);
        return true;
    } catch (err) {
        console.error(`[ProjectGremlin] Failed to inject system prompt for ${promptId}:`, err);
        return false;
    }
}

async function clearGremlinSystemPrompt(promptId) {
    const context = getContext();
    if (!context || typeof context.setExtensionPrompt !== 'function') {
        return;
    }
    try {
        await context.setExtensionPrompt(promptId, '', 0, 0, false, extension_prompt_roles.USER);
    } catch (err) {
        console.warn(`[ProjectGremlin] Failed to clear system prompt ${promptId}:`, err);
    }
}

export async function executeGen(promptText, options = {}) {
    if (typeof window.isAppReady === 'undefined' || !window.isAppReady) {
    ...
    const { useSystemPrompt = false, systemPromptId = null, systemPromptContent = null, triggerText = null } = options;

    if (useSystemPrompt && systemPromptId && systemPromptContent !== null) {
        const injected = await injectGremlinSystemPrompt(systemPromptId, systemPromptContent);
        if (!injected) {
            console.warn(`[ProjectGremlin] System prompt injection failed for ${systemPromptId}; falling back to /gen.`);
        } else {
            console.log(`[ProjectGremlin] Injected system prompt for ${systemPromptId}.`);
        }
    }

    // When using system prompt injection, send a minimal trigger so the model
    // responds to the system instructions. Otherwise, send the full prompt as before.
    const contentToSend = (useSystemPrompt && systemPromptContent !== null) ? (triggerText || 'Begin.') : promptText;
    const script = `/gen ${JSON.stringify(contentToSend)} |`;
    ...
    try {
        ...
        if (useSystemPrompt && systemPromptId) {
            await clearGremlinSystemPrompt(systemPromptId);
            console.log(`[ProjectGremlin] Cleared system prompt for ${systemPromptId}.`);
        }
    }
}
...
export async function runGremlinPlanningPipeline() {
    ...
    // Defensive cleanup: clear any Gremlin system prompts that may have been
    // left injected from a previous (possibly errored) run, so a fresh pipeline
    // starts with a clean prompt state.
    await clearGremlinSystemPrompt('gremlin_papa');
    await clearGremlinSystemPrompt('gremlin_mama');
    for (const twin of ['vex', 'vax']) {
        for (let k = 1; k <= (settings.gremlinTwinsIterations || 3) * 2; k++) {
            await clearGremlinSystemPrompt(`gremlin_twin_${twin}_${k}`);
        }
    }
    ...
    if (settings.gremlinPapaEnabled) {
        ...
        // Inject Papa's system-level instructions via the system role so the model
        // sees them in its native chat template, then send a minimal trigger.
        const papaResult = await executeGen(blueprintInstruction, {
            useSystemPrompt: true,
            systemPromptId: 'gremlin_papa',
            systemPromptContent: blueprintInstruction,
            triggerText: 'Produce the blueprint now, following all directives above.',
        });
    ...
    if (settings.gremlinTwinsEnabled) {
        ...
        if (!await applyGremlinEnvironment('twins')) {
            ...
            for (let i = 1; i <= numTwinIterations * 2; i++) {
                ...
                const twinSystemBase = isVexTurn ? vexPromptBase : vaxPromptBase;
                ...
                // The twin's role/persona is placed in the system role (injected),
                // and the per-iteration context (blueprint + previous sparks) is
                // sent as a user message. This is robust across model templates.
                const twinSystemContent = `[OOC: ${twinSystemBase}\n\nYou are ${currentTwin}, the imaginative Twin Gremlin. Provide a concise note (1-2 sentences) with a fresh, creative idea or concept. Don't hold back! ONLY provide the idea text, no other commentary.]`;
                const twinUserContext = `**Papa's Current Blueprint (${blueprintSource}):**\n${blueprint}\n---\n**Previous Twin Ideas (if any):**\n${twinDeliberations || 'None.'}\n---\n**Your Task (as the imaginative ${currentTwin}):**\nGet inspired!`;
                const twinNote = await executeGen(twinUserContext, {
                    useSystemPrompt: true,
                    systemPromptId: `gremlin_twin_${currentTwin.toLowerCase()}_${i}`,
                    systemPromptContent: twinSystemContent,
                    triggerText: 'Provide your creative spark now.',
                });
    ...
    if (settings.gremlinMamaEnabled) {
        ...
        if (!await applyGremlinEnvironment('mama')) {
            ...
            // Inject the full Mama template as a system prompt (so the role
            // instructions are seen in the model's native template), and pass
            // the actual blueprint/sparks as the user-side trigger context.
            const mamaSystemContent = mamaPrompt; // full template with placeholders replaced
            const mamaUserContext = `**Source Blueprint (${blueprintSource}):**\n${blueprint}\n\n**Twins' Creative Sparks (if any):**\n${twinDeliberations || 'None.'}\n\n---\nProduce the final, fully compliant, audited, and polished blueprint now. Output ONLY the blueprint text.`;
            const mamaResult = await executeGen(mamaUserContext, {
                useSystemPrompt: true,
                systemPromptId: 'gremlin_mama',
                systemPromptContent: mamaSystemContent,
                triggerText: 'Produce the final blueprint now.',
            });
            if (!mamaResult.trim()) {
